...
exports.create = (req, res) => {
  const { name, image, description, listId, rating, list } = req.body;
  //image data
  ...
  if (image) {
    s3.upload(params, (err, data) => {
      ...
      restaurant.image.url = data.Location;
      restaurant.image.key = data.Key;
    ...
    restaurant.image.key = "";
    restaurant.postedBy = req.user._id;
    //save to db
    restaurant.save((err, success) => {
      if (err) {
        console.log(err);
        res.status(400).json({ error: "error saving restaurant to db" });
      }
...
exports.remove = (req, res) => {
  const { slug } = req.params;
  console.log(slug);
  Restaurant.findOneAndRemove({ slug }).exec((err, data) => {
    const deleteParams = {
      Bucket: "foodiediary",
      Key: `${data.image.key}`,
    };
